...
public class GameEngine implements ValueFunction {
    ...
    //1. jeśli wstawimy pionek, to uzyskamy linię z 4 kamieni - 20pkt.
    ...
    public int[] makeMove(Game game){
        ...
        // Sprawdza wszystkie wolne pola na planszy
        if (filled>0 && empty == 0) {
            for (int i = 0; i<10;i++)
                for (int j=0; j<10;j++){
        ...
        // Sprawdza wolne pola w okolicy ostatnio polozonego pionka
        } else {
            for (int i = lastX -1; i <=lastX + 1; i++)
                for (int j = lastY -1; j<=lastY +1; j++ ){
